...
def test_messaging_settings_connection_url_default_none() -> None:
    assert RedisMessagingSettings().connection_url is None


def test_messaging_settings_connection_url_from_env(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("PREFECT_REDIS_MESSAGING_CONNECTION_URL", SENTINEL_URL)
    assert RedisMessagingSettings().connection_url == SENTINEL_URL


async def test_async_redis_from_settings_sentinel_url() -> None:
    settings = RedisMessagingSettings(connection_url=SENTINEL_URL)
...
async def test_async_redis_from_settings_single_node_url() -> None:
    settings = RedisMessagingSettings(connection_url="redis://cache:6380/2")
    client = async_redis_from_settings(settings)
...
async def test_get_async_redis_client_uses_connection_url(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("PREFECT_REDIS_MESSAGING_CONNECTION_URL", SENTINEL_URL)
...
async def test_get_async_redis_client_explicit_host_overrides_url(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("PREFECT_REDIS_MESSAGING_CONNECTION_URL", SENTINEL_URL)
    client_module._client_cache.clear()
    try:
        ...
        assert not isinstance(client.connection_pool, AsyncSentinelPool)
